/* gta02 machine plugin
 *
 * (C) 2008 by Vladimir Ananiev, based on  gta01 plugin
 *
...
static int gta02_power(struct gsmd *g, int power)
{
...
static int gta02_detect(struct gsmd *g)
{
	/* FIXME: do actual detection of machine if we have multiple machines */
	return 1;
}

static int gta02_init(struct gsmd *g, int fd)
{
	/*
