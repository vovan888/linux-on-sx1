	...
	case GSMD_TOA_TON_ALPHANUMERIC:
		for (i = 0; ((i * 7 + 3) >> 2) < src[0]; i++)
			dst->number[i] =
			    ((src[2 + ((i * 7 + 7) >> 3)] <<
			      (7 - ((i * 7 + 7) & 7))) |
			     (src[2 + ((i * 7) >> 3)] >> ((i * 7) & 7))) & 0x7f;
		break;
	default:
		for (i = 0; i < src[0]; i++)
			dst->number[i] = '0' + ((src[2 + (i >> 1)] >> ((i << 2) & 4)) & 0xf);
	}
	dst->number[i] = 0;
...
int sms_pdu_to_msg(struct gsmd_sms_list *dst, const u_int8_t * src, int pdulen, int len)
{
	int i, vpf;
...
int sms_pdu_make_smssubmit(char *dest, const struct gsmd_sms_submit *src)
{
	...
	header[pos++] = GSMD_SMS_TP_MTI_SUBMIT | (0 << 2) |	/* Reject Duplicates: 0 */
	    ...
	    (src->payload.has_header ? GSMD_SMS_TP_UDHI_WITH_HEADER :
	     GSMD_SMS_TP_UDHI_NO_HEADER) | GSMD_SMS_TP_RP_NOT_SET;
	...
	len = sms_data_bytelen(src->payload.coding_scheme, src->payload.length);
...
int sms_pdu_decode_dcs(struct gsmd_sms_datacodingscheme *dcs, const u_int8_t * data)
{
	...
	case 3:		/* pattern 1111 xxxx */
		...
		if ((data[pos] & 0x30) == 0x30) {
			...
			/* bit 3 */
			dcs->mwi_active = (data[pos] & 0x08) ? ACTIVE : NOT_ACTIVE;
			/* bit 2 is reserved */
			/* bits 1-0 */
