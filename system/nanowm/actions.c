...
static GR_BOOL PtInRect(GR_RECT * prc, GR_SIZE x, GR_SIZE y)
{
...
void closebutton_exposure(win * window, GR_EVENT_EXPOSURE * event)
{
	...
	GrBitmap(window->wid, buttonsgc, 0, 0, TITLE_BAR_HEIGHT,
		 TITLE_BAR_HEIGHT, window->active ? closebutton_pressed : closebutton_notpressed);
}
...
void topbar_mousemoved(win * window, GR_EVENT_MOUSE * event)
{
	...
	GrMoveWindow(window->pid, event->rootx - pos->x, event->rooty - pos->y);
