...
*
*  handles messages sent to modem
*  channel 4 (AMUX::104)
...
#define LSOCKET	"/tmp/sx_ext"	// local socket to listen on
#define MAXMSG	64
...
static int	fd_mux; // file descriptor for /dev/mux3, should be opened blocking
...
int InitPort(void)
{
	...
	if (fd_mux < 0)
	{
		perror ("Error: open /dev/mux3 failed!");
		exit (-1);
	}
// set port settings
	cfmakeraw(&options);
...
int PingIpcL(void)
{
	char req[6];
	char reply[6];
	// IpcHelper::PackPowerUpPingReq(TDes8 &)
...
//			ret =
		...
		case RAG_ModeTransition:
			ret = ipc_message_char( IPC_GROUP_RAGBAG, RAG_ModeTransition, data[0]);
			break;
	...
	if((the_pid=fork()) < 0) {
		...
		if(the_pid!=0) {
			if (wait_for_daemon_status) {
				wait(NULL);
				fprintf(stderr, "ipc_extension startup failed. See syslog for details.\n");
				exit(1);
			}
...
//-----------------------------------------------------
	/* Startup Init IPC */
	startup.pingok = PingIpcL();	// ping connection with EGold

	startup.reason = StartupReason();	//Get startup reason
	...
	if(!startup.rtccheck){
		...
		if(!res){
			...
			if(stime(&loc_time) < 0)
				perror("stime:");
			syslog(LOG_DEBUG," local time set = %s",asctime(&modem_time));
		}
	}

	if( SimGetDomesticLanguage(&cc) >=0 ) {
		startup.sim_domestic_lang = cc;
	...
	startup.sw_reason = SWStartupReason();	//PowerUpGetSwStartupReasonReq;
	...
	startup.selftest = SelfTest(0x0B);	// 0x0B = unknown constant
	syslog(LOG_DEBUG,"SelfTest = %d",startup.selftest);

	//RagbagSetDosAlarmReq(00000);
