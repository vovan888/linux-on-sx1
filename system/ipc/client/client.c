...
static inline void *debug_calloc(int size, int count)
{
	g_malloc++;
	return (calloc(size, count));
}

static inline void debug_free(void *ptr)
{
	g_malloc--;
	return (free(ptr));
}
...
static inline void pkt_free(pkt_buff_t * buf)
{
	if (!buf)
		return;
	if (buf->packet)
...
static int client_ServerDied(void)
{
...
static void client_QueuePacket(pkt_buff_t * packet)
{
...
static int client_DequeuePacket(pkt_buff_t ** ret)
{
...
static int client_SendToServer(int sock, unsigned char *data, int len)
{
...
int client_GetFromServer(int sock, pkt_buff_t ** ret_buffer)
{
...
static int client_WaitForData(int sock, int timeout)
{
	...
	*pkt = 0;
	...
	do {
		...
		if (data->header.type == CL_PKT_MESSAGE) {
			...
			DPRINT
			    ("While waiting for a %d, a Message packet was queued\n",
			     packet_type);
			qcount++;
		} else
...
static int local_Register(char *name, int inflags, int *outflags)
{
...
int ClRegister(char *name, int *flags)
{
	return (local_Register(name, CL_NORMAL_CLIENT, flags));
}

int ClReconnect(char *name)
{
	g_socket = 0;
...
int ClStartApp(char *name, char *args, int flags, int timeout)
{
...
int ClSpawnApp(char *name, char *args)
{
...
int ClLookupName(int id, char *name, int *len)
{
...
int ClGetMessage(void *msg, int *len, unsigned short *src)
{
	...
	if (client_DequeuePacket(&buffer) == 0) {
		...
		if (rlen == 0) {
			printf
			    ("CLCLIENT:  client_GetFromServer() returned 0\n");
...
int ClGetNextMessage(void *msg, int *len)
{
	...
	if (!client_DequeuePacket(&buffer)) {
		int ret =
		    client_WaitForServer(g_socket, CL_PKT_MESSAGE, &buffer, 0);
...
int ClLogMessage(int level, char *message)
{
...
int ClLogMessage(int level, char *name)
{
...
DLLEXPORT int ClGetServerIdByGroup(unsigned short group_id)
{
	...
	return ClFindApp(server_names[serverid]);
}
